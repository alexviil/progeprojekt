...
class Ai:
    def __init__(self, actors, actors_containers, items, effect_volume):
        self.actors = actors
    ...
    def move_randomly(self, creature):
        creature.control(libt.random_get_int(0, -1, 1), libt.random_get_int(0, -1, 1), self.actors, self.actors_containers, self.items, self.effect_volume)
    ...
    def chase_player(self, creature, creature_location, player_location):
        if player_location[0] < creature_location[0]:
            if player_location[1] < creature_location[1]:
                creature.control(- 1, - 1, self.actors, self.actors_containers, self.items, self.effect_volume)
            elif player_location[1] > creature_location[1]:
                creature.control(- 1, 1, self.actors, self.actors_containers, self.items, self.effect_volume)
            else:
                creature.control(- 1, 0, self.actors, self.actors_containers, self.items, self.effect_volume)
            ...
            if player_location[1] < creature_location[1]:
                creature.control(0, - 1, self.actors, self.actors_containers, self.items, self.effect_volume)
            elif player_location[1] > creature_location[1]:
                creature.control(0, 1, self.actors, self.actors_containers, self.items, self.effect_volume)
